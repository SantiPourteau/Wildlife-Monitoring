#Folder is called dataset_4channels which has two subfolders images and labels, the images and annotations are stored in the respective subfolders.
...
path = '../../data_yolo/dataset_4channels_' + file_type + '/images/'
train_path = '../../data_yolo/dataset_4channels_' + file_type + '/train/'
val_path = '../../data_yolo/dataset_4channels_' + file_type + '/val/'
test_path = '../../data_yolo/dataset_4channels_' + file_type + '/test/'
...
for image in images:
    if image in train:
        ...
        shutil.copyfile('../../data_yolo/dataset_4channels_' + file_type + '/labels/' + image.split('.')[0] + '.txt', train_path + 'labels/' + image.split('.')[0] + '.txt')
    elif image in val:
        shutil.copyfile(path + '/' + image, val_path + 'images/' + image)
        shutil.copyfile('../../data_yolo/dataset_4channels_' + file_type + '/labels/' + image.split('.')[0] + '.txt', val_path + 'labels/' + image.split('.')[0] + '.txt')
    elif image in test:
        shutil.copyfile(path + '/' + image, test_path + 'images/' + image)
        shutil.copyfile('../../data_yolo/dataset_4channels_' + file_type + '/labels/' + image.split('.')[0] + '.txt', test_path + 'labels/' + image.split('.')[0] + '.txt')